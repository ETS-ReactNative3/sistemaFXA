...
import DefaultFormik2 from './DefaultFormik2';

const DefaultData2 = (props) => {
    ...
    const onRowEditComplete1 = (e) => {
        ...
        if(!newDataArray[1]){
            props.toast.current.show({ severity: 'error', summary: 'Error', detail: 'El monto es obligatorio', life: 3000 })
        }else if(!/^\d+$/.test(newDataArray[1])){
            props.toast.current.show({ severity: 'error', summary: 'Error', detail: `El monto debe ser un número`, life: 3000 })
        }else if(!(parseInt(newDataArray[1])>=50000)){
            props.toast.current.show({ severity: 'error', summary: 'Error', detail: `El monto debe ser mayor o igual a $50.000`, life: 3000 })
        }else{
            service.update(newDataArray[0], {monto:newDataArray[1]}).then(res=>{
                if(res.status===201){
    ...
    const formikEmpresa = new DefaultFormik2()
    ...
    const formatterPeso = new Intl.NumberFormat('es-CO', {
      ...
      const bodyMonto = (e) =>{
          e = Object.values(e)
          return <span>{formatterPeso.format(parseInt(e[1]))}</span>
      }
    ...
    return <div>
       ...
       <DataTable selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)} editMode="row" onRowEditComplete={confirm1} value={data} paginator className="p-datatable-customers" rows={10}
        ...
        header={header1} paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" emptyMessage="No se encontro información" currentPageReportTemplate="Registros {first} a {last} de un total de {totalRecords}">
            <Column filter editor={(options) => textEditor(options)} showFilterMenu={false} body={e=>bodyMonto(e)} field={`${props.nombre}`} header="Monto" sortable/>
            ...
            <div className='w-full text-center'>
                ...
                <div className="col-12 mt-5">
                    <span className="p-float-label">
                        <InputText name={`monto`} type="text"  className={classNames({ 'p-invalid': isFormFieldValid(`monto`) })+' w-full'}  value={formik.values.monto} onChange={formik.handleChange}></InputText> 
                        <label>{props.name}:</label>
                    </span>
                    <div>{getFormErrorMessage(`monto`)}</div>
                </div>
