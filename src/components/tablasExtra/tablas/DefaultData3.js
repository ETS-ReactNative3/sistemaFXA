...
import DefaultFormik3 from './DefaultFormik3';

const DefaultData3 = (props) => {
    ...
    const onRowEditComplete1 = (e) => {
        ...
        if(!newDataArray[1]){
            props.toast.current.show({ severity: 'error', summary: 'Error', detail: 'La talla es obligatoria', life: 3000 })
        }else if(!(newDataArray[1].length >= props.minMax[0] && newDataArray[1].length <= props.minMax[1])){
            props.toast.current.show({ severity: 'error', summary: 'Error', detail: `La talla debe tener de ${props.minMax[0]} a ${props.minMax[1]} caracteres`, life: 3000 })
        }else{
            service.update(newDataArray[0], {talla:newDataArray[1]}).then(res=>{
                if(res.status===201){
    ...
    const formikEmpresa = new DefaultFormik3()
            ...
            <div className='w-full text-center'>
                ...
                <div className="col-12 mt-5">
                    <span className="p-float-label">
                        <InputText name={`talla`} type="text"  className={classNames({ 'p-invalid': isFormFieldValid(`talla`) })+' w-full'}  value={formik.values.talla} onChange={formik.handleChange}></InputText> 
                        <label>{props.name}:</label>
                    </span>
                    <div>{getFormErrorMessage(`talla`)}</div>
                </div>
